...
def run_batch(x, y, model, optimizer):
    ...
    l_kl /= bs * 3 * args.dim
    loss = args.re * l_rec + args.ce * l_ce + args.kl * l_kl
    loss.backward()
    optimizer.step()

    prec, _, _, _ = accuracy(out.data, y.data, topk=(1, 5))
    return loss, l_rec, l_ce, l_kl, prec
...
def main(args):
    ...
    # Model
    print('\n[Phase 2] : Model setup')
    model = CVAE_cifar_lowdim(d=feature_dim, z=CNN_embed_dim)
    ...
    for epoch in range(start_epoch, start_epoch + args.epochs):
        ...
        for batch_idx, (x, y) in enumerate(trainloader):
            ...
            if (batch_idx + 1) % 30 == 0:
                sys.stdout.write('\r')
                sys.stdout.write(
                    '| Epoch [%3d/%3d] Iter[%3d/%3d]\t\t Loss_rec: %.4f Loss_ce: %.4f Loss_kl: %.4f Acc_x@1: %.3f%%   '
                    % (epoch, args.epochs, batch_idx + 1,
                       len(trainloader),  loss_rec.avg, loss_ce.avg, loss_kl.avg, top1.avg))
        scheduler.step()
        if epoch % 10 == 1:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--real', default=0.5, type=float, help='diverse')
