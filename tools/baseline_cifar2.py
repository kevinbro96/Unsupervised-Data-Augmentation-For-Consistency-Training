    ...
    with torch.no_grad():
        for batch_idx, (X, y) in enumerate(cifar10_dataloader):
            if batch_idx >= batch_num:
                ...
                grid_Xi = torchvision.utils.make_grid((X-rx)[:batch_size].data, nrow=8, padding=2, normalize=True)
                wandb.log({"_Batch_{batch}_Xi.jpg".format(batch=batch_idx): [
                    wandb.Image(grid_Xi)]}, commit=False)
                grid_X_Xi = torchvision.utils.make_grid((rx[:batch_size]).data, nrow=8, padding=2,
                                                        normalize=True)
...
def test(epoch, model, testloader):
    ...
    with torch.no_grad():
        for batch_idx, (x, y) in enumerate(testloader):
            ...
            out, z, rx = model(x)
            acc_gx = 1 - F.mse_loss(torch.div(x-rx, norm.unsqueeze(1).unsqueeze(2).unsqueeze(3)), \
                                    torch.div(x, norm.unsqueeze(1).unsqueeze(2).unsqueeze(3)), \
                                    reduction='sum') / 100
            acc_rx = 1 - F.mse_loss(torch.div(rx, norm.unsqueeze(1).unsqueeze(2).unsqueeze(3)), \
                                    torch.div(x, norm.unsqueeze(1).unsqueeze(2).unsqueeze(3)), \
                                    reduction='sum') / 100
...
def run_batch(x, y, model, optimizer):
    x, y = x.cuda(), y.cuda().view(-1, )
    x, y = Variable(x), Variable(y)
    out, z, rx = model(x)

    optimizer.zero_grad()
    l_rec = F.mse_loss(torch.zeros(x.size()).cuda(), rx)
...
def main(args):
    ...
    # Model
    print('\n[Phase 2] : Model setup')
    model = CVAE_cifar_baseline2(d=feature_dim, z=CNN_embed_dim)
